    ...
    <div className="min-h-screen overflow-hidden">
      <section className="hero-grid relative px-6 pb-24 pt-36 md:pb-32 md:pt-44">
        ...
        <div className="relative mx-auto grid max-w-7xl items-center gap-16 lg:grid-cols-[1.04fr_0.96fr]">
          <div className="max-w-3xl">
            ...
            <p className="animate-fade-up delay-200 mt-7 max-w-2xl text-base leading-8 text-slate-300/70 md:text-lg">
              Turn everyday choices into measurable climate action. EcoStep gives you a clear carbon score, focused insights, and practical ways to improve.
            </p>
            <div className="animate-fade-up delay-300 mt-9 flex flex-col gap-3 sm:flex-row">
              <Link href="/calculator" className="primary-button group">Calculate my footprint <ArrowRight className="h-4 w-4 transition-transform group-hover:translate-x-1" /></Link>
            ...
            <div className="animate-fade-up delay-400 mt-8 flex flex-wrap gap-x-6 gap-y-3 text-sm text-slate-400">
          ...
          <div className="animate-fade-up delay-300 relative mx-auto w-full max-w-[590px]">
            ...
            <div className="dashboard-shell relative rounded-[2rem] p-3 sm:p-4">
              <div className="rounded-[1.45rem] border border-white/[0.07] bg-[#09140f]/95 p-5 sm:p-6">
                <div className="flex items-center justify-between">
                  <div><p className="text-xs font-medium uppercase tracking-[0.18em] text-slate-500">Your carbon overview</p><p className="mt-1 text-sm font-medium text-slate-200">Weekly footprint</p></div>
                ...
                <div className="mt-6 grid gap-4 sm:grid-cols-[1.05fr_0.95fr]">
                  <div className="metric-card flex min-h-[250px] flex-col items-center justify-center p-5">
                    <div className="carbon-ring relative flex h-40 w-40 items-center justify-center rounded-full">
                      <div className="flex h-[116px] w-[116px] flex-col items-center justify-center rounded-full border border-white/[0.06] bg-[#0a1711] shadow-inner"><span className="text-3xl font-semibold text-white">7.4</span><span className="mt-1 text-[11px] uppercase tracking-[0.16em] text-slate-500">tons / year</span></div>
                    </div>
                  ...
                  <div className="grid gap-3">
                    <div className="metric-card p-4">
                      <div className="flex justify-between text-xs"><span className="text-slate-400">Today&apos;s score</span><span className="text-emerald-300">Good</span></div>
                    ...
                    <div className="metric-card p-4"><div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-xl bg-amber-300/10 text-amber-300"><Plane className="h-5 w-5" /></div><div className="flex-1"><p className="text-xs text-slate-500">Largest impact</p><p className="mt-0.5 text-sm font-medium text-slate-200">Transport</p></div><span className="text-sm font-semibold text-white">38%</span></div></div>
                  </div>
                </div>
                <div className="mt-4 grid grid-cols-3 gap-3">
                  {[['12', 'actions'], ['6', 'day streak'], ['24kg', 'CO2 saved']].map(([value, label]) => <div key={label} className="metric-card px-3 py-3 text-center"><div className="text-base font-semibold text-slate-100">{value}</div><div className="mt-0.5 text-[10px] uppercase tracking-wider text-slate-500">{label}</div></div>)}
                </div>
              </div>
            </div>
            <div className="floating-chip floating-chip-left"><div className="flex h-9 w-9 items-center justify-center rounded-lg bg-emerald-300/10 text-emerald-300"><Leaf className="h-4 w-4" /></div><div><p className="text-[10px] uppercase tracking-wider text-slate-500">This week</p><p className="text-xs font-semibold text-white">3 goals completed</p></div></div>
        ...
        <div className="relative mx-auto mt-24 grid max-w-7xl grid-cols-2 gap-px overflow-hidden rounded-2xl border border-white/[0.06] bg-white/[0.06] md:grid-cols-4" data-reveal>
          {stats.map(([value, label]) => <div key={label} className="bg-[#07110c]/90 px-5 py-6 text-center backdrop-blur-xl"><div className="text-2xl font-semibold text-white">{value}</div><div className="mt-1 text-xs leading-5 text-slate-500">{label}</div></div>)}
        </div>
      </section>

      <section id="how-it-works" className="section-shell px-6 py-28">
        <div className="mx-auto max-w-7xl">
          <div className="section-heading" data-reveal><span className="eyebrow">Simple by design</span><h2>From uncertainty to a clear action plan.</h2><p>EcoStep turns complex emissions data into a focused experience you can understand and use.</p></div>
          <div className="mt-16 grid gap-5 lg:grid-cols-3">
            {steps.map((step, index) => <article key={step.step} className="feature-card group" data-reveal style={{ transitionDelay: `${index * 100}ms` }}><div className="flex items-center justify-between"><div className="icon-box"><step.icon className="h-5 w-5" /></div><span className="text-xs tracking-[0.2em] text-slate-600">{step.step}</span></div><h3 className="mt-8 text-xl font-semibold text-white">{step.title}</h3><p className="mt-3 text-sm leading-7 text-slate-400">{step.description}</p><div className="mt-8 h-px bg-gradient-to-r from-emerald-300/30 to-transparent" /></article>)}
          </div>
        </div>
      </section>
